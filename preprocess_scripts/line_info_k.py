        ...
        for file in tqdm(file_list):
            ...
            for story in tqdm(glob(story_list)):
                ...
                for data in tqdm(glob(story + "/*")):
                    if f_name + story.lstrip(look_up_file) + "line_info_k.csv" in glob(f_name+"*.csv"):
                        pass
                    elif len(glob(story + "/*")) == 1:
                        ono_counter = search_inside_sentence(data,ono_lis_st,ono_counter,"line_info_k",True)
                    else:
                        if self.exclude_strings in data:
                           pass
                        else:
                            ono_counter = search_inside_sentence(data, ono_lis_st, ono_counter, "line_info_k",False)
                if f_name + story.lstrip(look_up_file) + "line_info_k.csv" in glob(f_name + "*.csv"):
                    print(f_name + story.lstrip(look_up_file) + "line_info_k.csv")
                    pass
                else:
                    ...
                    df.to_csv(f_name + story.lstrip(look_up_file) + "line_info_k.csv", index=False)