            ...
            for story in tqdm(glob(story_list)):
                ...
                for data in tqdm(glob(story + "/*")):
                    if f_name + story.lstrip(look_up_file) + "line_info_n.csv" in glob(f_name+"*.csv"):
                        ...
                        ono_counter = search_inside_sentence(data,ono_lis_st,ono_counter,"line_info_n",True)
                    else:
                        if self.exclude_strings in data:
                           pass
                        else:
                            ono_counter = search_inside_sentence(data, ono_lis_st, ono_counter, "line_info_n",False)
                if f_name + story.lstrip(look_up_file) + "line_info_n.csv" in glob(f_name + "*.csv"):
